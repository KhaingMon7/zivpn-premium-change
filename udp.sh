...
# ===== UDP CONNECTION TRACKING TIMEOUT FIX (module check) =====
...
if [ -f /proc/sys/net/netfilter/nf_conntrack_udp_timeout ]; then
    ...
    echo -e "${Y}⚠️ nf_conntrack module not available. UDP timeout settings skipped (not critical).${Z}"
fi
...
iptables -t raw -I PREROUTING -p udp --dport 5667 -j NOTRACK 2>/dev/null
iptables -t raw -I PREROUTING -p udp --dport 6000:19999 -j NOTRACK 2>/dev/null
iptables -t raw -I OUTPUT -p udp --sport 5667 -j NOTRACK 2>/dev/null
...
# UFW Rules (commented to avoid blocking yourself)
